...
class Comment extends CommentModel
{
     ...
     * 获取指定商品评价列表
     */
    public function getProductCommentList($product_id, $scoreType, $limit = 15)
    {
        // 筛选条件
        $filter = [
    ...
    private function createCommentData($user_id, $order_id, $productList, $formData)
    {
        ...
        foreach ($productList as $product) {
            ...
            !empty($item['content']) && $data[$product['order_product_id']] = [
                'express_score' => isset($item['express_score']) ? $item['express_score'] : 5,
                'server_score' => isset($item['server_score']) ? $item['server_score'] : 5,
                'describe_score' => isset($item['describe_score']) ? $item['describe_score'] : 5,
