        ...
        <el-form-item label="商品分类">
          <!-- <el-select v-model="formInline.category_id" placeholder="请选择商品分类">
            <template v-for="cat in cateList" :key="cat.category_id" >
              ...
              <template v-if="cat.child && cat.child.length > 0">
        ...
        <el-table size="small" :data="productList" border style="width: 100%" highlight-current-row v-loading="loading" @selection-change="tableCurrentChange">
          <el-table-column width="70" label="商品图片">
            <template #default="{ row }">
              <img :src="row.image && row.image[0].file_path" width="30" height="30" />
            </template>
          </el-table-column>
          ...
          <el-table-column width="80" label="单选" v-if="!islist">
            <template #default="{ row }">
              <el-button size="mini" v-if="row.noChoose" @click="SingleFunc(row)">选择</el-button>
...
<script>
import { chooseLists } from '@/api/product.js';
export default {
  data() {
    return {
      /*是否加载完成*/
