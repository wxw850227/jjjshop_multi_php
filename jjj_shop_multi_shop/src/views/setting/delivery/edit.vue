<template>
  <div class="product-add">
    ...
    >
      ...
      <el-form-item label="配送区域及运费">
        <el-table :data="form.rule" border style="width: 100%">
          <el-table-column label="可配送区域">
            <template #default="scope">
              <div class="area-list">
                ...
                <span
                  v-else
                  v-for="(province, index) in scope.row.showData"
                  :key="index"
                  class="pr16"
                >
                  {{ province.name }}
                  <template v-if="!province.isAllCitys">
                    ...
                    <span class="am-link-muted gray9">
                      <em
                        v-for="(city, city_index) in province.citys"
                        :key="city_index"
                      >
                        <span>{{ city.name }}</span>
                        <span v-if="city_index + 1 < province.citys.length"
                          >、</span
                        >
                      </em>
                    </span>
              ...
              <a
                ...
                @click="el = editAreaClick(scope.$index)"
                >编辑</a
              >
            </template>
          </el-table-column>
          ...
          <el-table-column
            prop="additional"
            :label="tableHeadName.additional"
            width="100px"
          >
            <template #default="scope">
          ...
          <el-table-column
            prop="additional_fee"
            :label="tableHeadName.additional_fee"
            width="100px"
          >
            <template #default="scope">
          ...
          <el-table-column label="操作" width="60px">
            <template #default="scope">
              <a
                href="javascript:void(0);"
                @click="deleteAreaClick(scope.$index)"
                >删除</a
              >
            </template>
          </el-table-column>
        ...
        <div>
          <el-button @click="addAreaClick" link type="primary" size="small"
            >+点击添加可配送区域和运费</el-button
          >
        </div>
      </el-form-item>
    ...
    <Areas
      ...
      :areaModel="areaModel"
      :areas="options"
      @closeArea="closeAreaFunc"
    ></Areas>
  </div>
</template>

<script>
import { deliveryDetail, editDelivery } from "@/api/setting.js";
  ...
  methods: {
      ...
      deliveryDetail(
        ...
        .then((data) => {
          if (self.delivery_id > 0) {
            ...
            self.form.rule = [];
            self.form.radio = self.form.method.value.toString();
    ...
    closeAreaFunc(e) {
      ...
      e.this_area.forEach(function (province, index) {
        if (
          (province.checked == true || province.indeterminate == true) &&
          province.index.indexOf(self.areaModel.index) != -1
        ) {
    ...
    deleteAreaClick(e) {
      ...
      self.options.forEach(function (province) {
        if (
          self.form.rule[e].province &&
          self.inArray(province["value"], self.form.rule[e].province)
        ) {
      ...
      self.$refs.form.validate((valid) => {
        if (valid) {
          ...
          for (let key in form.rule) {
            if (
              !form.rule[key].hasOwnProperty("first") ||
              !(form.rule[key].first > 0)
            ) {
              ElMessage({
                message: "首件/首重不能为空",
                type: "error",
            ...
            if (
              !form.rule[key].hasOwnProperty("first_fee") ||
              form.rule[key].first_fee == ""
            ) {
              ElMessage({
                message: "运费不能为空",
                type: "error",
            ...
            if (
              !form.rule[key].hasOwnProperty("additional") ||
              !(form.rule[key].additional > 0)
            ) {
              ElMessage({
                message: "续件/续重不能为空",
                type: "error",
            ...
            if (
              !form.rule[key].hasOwnProperty("additional_fee") ||
              form.rule[key].additional_fee == ""
            ) {
              ElMessage({
                message: "续费不能为空",
                type: "error",
