    ...
    <div class="product-content">
      <div class="table-wrap">
        <el-tabs v-model="activeName" @tab-change="handleClick">
         ...
         <el-tab-pane name="0">
           <template #label>
             <span> {{ '进行中(' + (arr[0] ? arr[0].total : 0) + ')' }}</span>
           </template>
         </el-tab-pane>
         <el-tab-pane name="10">
           <template #label>
            <span> {{ '已拒绝(' + (arr[10] ? arr[10].total : 0) + ')' }}</span>
           </template>
         </el-tab-pane>
         <el-tab-pane name="20">
           <template #label>
             <span>{{ '已完成(' + (arr[20] ? arr[20].total : 0) + ')' }}</span>
           </template>
         </el-tab-pane>
         <el-tab-pane name="30">
           <template #label>
             <span>{{ '已取消(' + (arr[30] ? arr[30].total : 0) + ')' }}</span>
           </template>
         </el-tab-pane>
        </el-tabs>
        <el-table :data="tableData" border style="width: 100%" v-loading="loading">
          ...
          <el-table-column prop="orderproduct.product_name" width="400" label="商品信息">
            <template #default="scope">
              <div class="d-s-c">
                <img v-img-url="scope.row.orderproduct.image.file_path" alt="" :width="50" />
          ...
          <el-table-column prop="user.nickName" label="买家">
            <template #default="scope">
              ...
              <br />
              <span v-if="scope.row.user" class="gray9">(ID:{{ scope.row.user.user_id }})</span>
            </template>
          </el-table-column>
          ...
          <el-table-column fixed="right" label="操作" width="110">
            <template #default="scope">
              <p>
                <el-button @click="Detail(scope.row.order_refund_id)" link type="primary" size="small">售后详情</el-button>
              </p>
              <p>
                <el-button v-if="scope.row.is_agree.value == 0" @click="Detail(scope.row.order_refund_id)" link type="primary"
                  size="small">去审核</el-button>
              </p>
              <p>
                <el-button
                  v-if="scope.row.type.value != 30 && scope.row.is_agree.value == 10 && scope.row.is_receipt == 0 && scope.row.is_user_send == 1"
                  @click="Detail(scope.row.order_refund_id)" link type="primary" size="small">
                  处理
                </el-button>
              </p>
            </template>
...
<script>
  import {orderrefund} from '@/api/order.js';
  export default {
    data() {
      return {
        /*是否加载完成*/
