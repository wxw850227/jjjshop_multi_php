<template>
    <el-dialog v-model="show" :title="dialogTitle" width="1100" :before-close="handleClose" class="noBg">
      <div>
        ...
        <el-card shadow="never">
              ...
              <!-- <el-form :model="form" label-width="120px" :rules="rules" ref="formRef">
                ...
                <el-table 
                  ...
                  <el-table-column type="selection" width="60" />
                    ...
                    <el-table-column prop="tableName" label="检查表名称" />
                    <el-table-column prop="categoryName" label="所属目录"  />
                    <el-table-column prop="level" label="检查表层级">
                      <template #default="{ row }">
                          {{ row.level == 1 ? '单级检查表' : row.level == 2 ? '多级检查表' : '' }}
                      </template>
                  </el-table-column>
            ...
            <div v-if="activeName == 'paper'">
              <el-table 
                    :data="data.paper.tableData"
                    @select="handSelectChange"
                    @select-all="allSelect('paperRef')"
                    border
                    ref="paperRef"
                    :max-height="400"
                  >
                  ...
                  <el-table-column prop="evaluationMethod" label="评估方式" width="80">
                    <template #default="{ row }">
                      {{ row.evaluationMethod == 0 ? '分值统计' : row.evaluationMethod == 1 ? '比例统计' : ''}}
                    </template>
                  </el-table-column>
                  ...
                  <el-table-column label="操作" width="140" fixed="right">
                    <template #default="{ row }">
                      <el-button type="warning" size="small" plain @click="clickPaperDetail(row)" >详情</el-button>
                      <el-button type="success" size="small" plain @click="clickPaperPreview(row)">预览</el-button>
                    </template>
                  </el-table-column>
              </el-table>
              <el-pagination @size-change="handleSizeChange" @current-change="handleCurrentChange" background
            ...
            <div v-if="activeName == 'unitCourse'">
              <el-table 
                    :data="data.unitCourse.tableData"
                    @select="handSelectChange"
                    @select-all="allSelect('unitCourseRef')"
                    border
                    ref="unitCourseRef"
                    :max-height="400"
                  >
            ...
            <div v-if="activeName == 'publicCourse'"> 
              <el-table 
                    :data="data.publicCourse.tableData"
                    @select="handSelectChange"
                    @select-all="allSelect('unitCourseRef')"
                    border
                    ref="publicCourseRef"
    ...
    <paperBasicDetail ref="paperBasicDetailRef" />
  ...
  import { dangercheckTableIndex } from "@/api/prevention/danger/danger.js";   
  import { coursewareIndex } from "@/api/course.js";
  import { indexPaper } from "@/api/paper";
  import { isArray } from "@/utils/validate";
  import addMyCheckIndex from "@/views/spot/manage/facility/my/addMyCheck/index.vue";
  import paperPreview from "@/views/education/paperPreview/index.vue";
  import paperBasicDetail from "@/views/education/paperBasicDetail.vue";
  ...
  export default defineComponent({
    ...
    props: {
      // unitCourse 单位课程库 publicCourse 公共课程库
    ...
    setup(props) {
      ...
      const getCompanyCheck = async ()=>{
        const { data: { records, total } } = await dangercheckTableIndex(state.data[state.activeName].queryData);
        state.data[state.activeName].tableData = records;
        state.data[state.activeName].total = total;
      }
      // 公共检查表
      ...
      const getPaper = async ()=>{
        const { data: { records, total } } = await indexPaper(state.data[state.activeName].queryData);
        state.data[state.activeName].tableData = records;
        state.data[state.activeName].total = total;
      }

      // 单位课程库
      const getUnitCourse = async ()=>{
        const { data: { records, total } } = await coursewareIndex(state.data[state.activeName].queryData);
        state.data[state.activeName].tableData = records;
        state.data[state.activeName].total = total;
      }
      ...
      const changTab = async ()=>{
        if(state.activeName == 'staff'){
          ...
          await getMangmentMy();
        }else if(state.activeName == 'paper'){
          await getPaper();
        }else if(state.activeName == 'unitCourse'){
          await getUnitCourse();
        }else if(state.activeName == 'publicCourse'){
          await getPublicCourse();
        }
    ...
    methods: {
      clickPaperDetail(row){
        this.paperBasicDetailRef.open(row);
      },
      clickPaperPreview(row){
        this.paperPreviewRef.open(row);
      },
      clickMangmentMyAdd(){
